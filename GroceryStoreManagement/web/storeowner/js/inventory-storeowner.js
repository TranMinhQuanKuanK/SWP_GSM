...
function printProductList(data) {
  ...
  for (i = 0; i < data.length; i++) {
    if (data[i].is_selling !== false) {
      ...
      //đoạn code của Hùng
      Add_bt.innerHTML =
        "<i class='far fa-share-square btn-inventory mr-2'></i>";
      Add_bt.setAttribute(
        "onclick",
        "addToPendingListByOwner(" + data[i].product_ID + ")"
      );
      //keythucssfdsd
      const foundInPending = Boolean(
        pendingList.filter((item) => item.product_ID === data[i].product_ID)
          .length
      );

      if (foundInPending === false) {
        Add_bt.innerHTML =
          "<i class='fas fa-plus-circle btn-inventory mr-2'></i>";
        Add_bt.setAttribute(
          "onclick",
          "addToPendingListByOwner(" + data[i].product_ID + ")"
        );
        ...
        Add_bt.innerHTML =
          "<i class='fas fa-plus-circle btn-inventory mr-2' style='opacity: 0.2;'></i>";
      ...
      //keeys thcusndfskjsjflkdjf;sjdfsdf
