...
function setTitle() {
  if (viewBy === "custom") {
    period.innerHTML =
      "giai đoạn " +
      formatDateTime(dateFrom.value) +
      " - " +
      formatDateTime(dateTo.value);
  } else {
  ...
  if (viewBy === "custom") {
    ...
    if (statCriterion === "financial") {
      ...
      if (viewBy === "quarter") {
        var firstMonthOfQuarter = format((new Date().getMonth() / 3) * 3 + 1);
        dateFrom.value =
          dateTo.value.substring(0, 5) + firstMonthOfQuarter + "-01";
      }
      if (viewBy === "year") {
...
function renderFinancialStatistic(financialStatistic) {
  ...
  document.getElementById("sum-revenue").innerHTML = eVietnam(
    financialStatistic.sumRevenue
  );
  document.getElementById("sum-cost").innerHTML = eVietnam(
    financialStatistic.sumCost
  );
  document.getElementById("sum-profit").innerHTML = eVietnam(
    financialStatistic.sumProfit
  );
...
function drawCurveTypes(chartData) {
  ...
  if (document.getElementById("empty-row-display").value === "hide") {
    ...
    if (emptyItems.length !== 0) {
      document.getElementById("chart-date-error").textContent =
        "Giai đoạn " +
        emptyItems[0] +
        " - " +
        emptyItems[emptyItems.length - 1] +
        " không có dữ liệu nên sẽ không được hiển thị trong biểu đồ";
      $("#warning-chart-toast").toast({
        delay: 3000
      });
      $("#warning-chart-toast").toast("show");
    }
  }
...
function centerChartTitle() {
  ...
  var sideBarWidth =
    document.getElementById("perfect-scrollbar").offsetWidth ||
    document.getElementById("perfect-scrollbar").clientWidth;
  var chartTitle = document.querySelector(
    "#chart_div > div > div:nth-child(1) > div > svg > g:nth-child(3) > text"
  );
  var centerXAxis =
    width / 2 - chartTitle.innerHTML.length - sideBarWidth - 110;
